  ...
  run(creep) {
    ...
    if (creep.memory.working === true) {
      const structure = creep.pos.findClosestByPath(FIND_STRUCTURES,
        {
        filter: (s) => {
          const roomSpawnsNotFull = (creep.room.energyAvailable < creep.room.energyCapacityAvailable) && creep.room.energyCapacityAvailable >= creep.carry.energy;
          const roomNoHaulers = Memory.rooms[creep.room.name].creepCount.haulerSpawning === 0 || Memory.rooms[creep.room.name].creepCount.haulerMining === 0;
          // console.log(Memory.rooms[creep.room.name].creepCount.haulerSpawning);
          // console.log(roomNoHaulers);
          if (roomSpawnsNotFull && roomNoHaulers) {
      ...
      //creep.getEnergy(false, true);
        ...
        if (sourceID) {
          ...
          //console.log(source.memory.freeSpaceCount);
